...
| WP-18 — V2 Campaign Trigger workflow                     | in_progress | Durable/resumable parent and child stages now execute bounded semantic discovery, retry-safe canonical entity resolution, bounded Candidate research, and relationship-first Qualification with frozen inputs, cached AI boundaries, deterministic scoring, and independently retryable fan-out; comparative ranking remains. |
...
- Guarded V2 canonical writes exist only for workspaces explicitly configured for the
  V2 workflow with canonical result writes and shadow mode disabled.
...
Candidate research, and Qualification are connected. Comparative ranking deliberately
fails closed. V2 routing therefore remains disabled until that adapter and the remaining
workflow controls are complete.
...
Comparative ranking remains disconnected and fail-closed. V2 routing therefore stays
disabled.
