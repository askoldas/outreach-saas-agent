...
| WP-18 — V2 Campaign Trigger workflow                     | in_progress | Durable/resumable parent and child stages plus bounded semantic discovery now freeze run-scoped Memory, Strategy, plans, routes, capabilities, initial and targeted Segment Runs, exact gap actions, settled query audit, provider facts, cumulative coverage, marginal yield, and aggregate continuation decisions; downstream entity, research, qualification, and ranking adapters remain. |
...
Canonical organization resolution, candidate research, qualification, and ranking
remain disconnected. V2 routing therefore stays disabled.
