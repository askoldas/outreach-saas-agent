...
| WP-18 — V2 Campaign Trigger workflow                     | in_progress | Durable/resumable stages now execute semantic discovery, entity resolution, Candidate research, relationship-first Qualification, and deterministic Comparative Ranking with frozen inputs and retry-safe persistence; pause, resume, cancel, progress reconciliation, and terminal-state controls remain. |
...
All Intelligence stages are now connected. V2 routing remains disabled while pause,
resume, cancel, progress reconciliation, and terminal-state controls are still pending.
