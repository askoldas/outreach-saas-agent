  ...
  return (
    <div className={styles.grid}>
      <PageHeader
        ...
        actions={<Badge tone={statusTone(draft.status)}>{statusLabel(draft.status)}</Badge>}
      />
      <section className={styles.twoColumn}>
        <Card>
          <CardHeader title="Draft editor" eyebrow={draft.language} />
          <div className={styles.cardBody}><DraftEditor draft={draft} /></div>
        </Card>
        <div className={styles.stack}>
          <Card>
            <CardHeader title="Recipient information" eyebrow="Public route" />
            <div className={styles.cardBody}>
              <p><strong>Lead:</strong> {lead?.company}</p>
              <p><strong>Route:</strong> {draft.recipientRoute}</p>
              <p><strong>Campaign:</strong> {campaign?.name}</p>
              <p><strong>Variant:</strong> {statusLabel(draft.variant)}</p>
            </div>
          </Card>
          <Card>
            <CardHeader title="Selected seller claims" eyebrow="Approved offer inputs" />
            <div className={styles.cardBody}><ul className={styles.pillList}>{draft.sellerClaims.map((item) => <li key={item}>{item}</li>)}</ul></div>
          </Card>
          <Card>
            <CardHeader title="Prospect evidence used" eyebrow="Internal grounding" />
            <div className={styles.cardBody}><ul className={styles.pillList}>{draft.evidenceUsed.map((item) => <li key={item}>{item}</li>)}</ul></div>
          </Card>
          <Card>
            <CardHeader title="Personalization warnings" eyebrow="Do not overstate" />
            <div className={styles.cardBody}><ul className={styles.feed}>{draft.warnings.map((warning) => <li key={warning}><strong>Warning</strong><p>{warning}</p></li>)}</ul></div>
          </Card>
        </div>
      </section>
    </div>
