...
const minimumFitScore = 60;

export async function evaluateLeadCandidates(
  campaign: Campaign,
  results: SearchResult[],
): Promise<EvaluatedLeadCandidate[]> {
  if (results.length === 0) {
    return [];
  }
  ...
  return (parsed.leads ?? [])
    .map((lead) => mapEvaluatedLead(lead, resultByUrl))
    .filter((lead): lead is EvaluatedLeadCandidate => Boolean(lead))
    .filter((lead) => lead.fitScore >= minimumFitScore)
...
function parseEvaluation(content: string): RawEvaluationResponse {
  ...
  try {
    ...
    throw new Error("OpenRouter returned an invalid lead evaluation response.");
  }
}
