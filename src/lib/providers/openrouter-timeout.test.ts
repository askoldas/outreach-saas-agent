...
test("OpenRouter uses a worker-safe timeout by default", () => {
  const previous = process.env.OPENROUTER_REQUEST_TIMEOUT_MS;
...
test("OpenRouter timeout is configurable with a safe minimum", () => {
  const previous = process.env.OPENROUTER_REQUEST_TIMEOUT_MS;
  process.env.OPENROUTER_REQUEST_TIMEOUT_MS = "75000";
  assert.equal(getOpenRouterTimeoutMs(), 75_000);
  process.env.OPENROUTER_REQUEST_TIMEOUT_MS = "1000";
  assert.throws(() => getOpenRouterTimeoutMs(), /must be at least 5000/);
  restore(previous);
});

function restore(value: string | undefined) {
  if (value === undefined) delete process.env.OPENROUTER_REQUEST_TIMEOUT_MS;
  else process.env.OPENROUTER_REQUEST_TIMEOUT_MS = value;
}
