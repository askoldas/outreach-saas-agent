-- Semantic discovery plans, coverage, gaps, and stopping. Apply after 20260728001000.
...
create table public.discovery_segments (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
...
create table public.discovery_source_plans (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  discovery_segment_id uuid not null references public.discovery_segments(id) on delete cascade,
...
create table public.discovery_segment_runs (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
...
add column discovery_segment_run_id uuid
references public.discovery_segment_runs(id) on delete cascade;

create index discovery_provider_executions_segment_run_idx
...
create table public.discovery_coverage_snapshots (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  discovery_run_id uuid not null references public.discovery_runs(id) on delete cascade,
...
create table public.discovery_gaps (
  ...
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  discovery_run_id uuid not null references public.discovery_runs(id) on delete cascade,
  discovery_segment_id uuid references public.discovery_segments(id) on delete cascade,
...
create table public.discovery_gap_actions (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  discovery_gap_id uuid not null references public.discovery_gaps(id) on delete cascade,
...
create table public.discovery_usage_events (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  discovery_run_id uuid not null references public.discovery_runs(id) on delete cascade,
  discovery_segment_run_id uuid references public.discovery_segment_runs(id) on delete cascade,
...
begin
  if tg_table_name = 'discovery_plans' then
  ...
  elsif tg_table_name in ('discovery_segments') then
    select workspace_id into expected_workspace_id from public.discovery_plans
    where id = new.discovery_plan_id;
  elsif tg_table_name = 'discovery_source_plans' then
    select workspace_id into expected_workspace_id from public.discovery_segments
  ...
  elsif tg_table_name = 'discovery_segment_runs' then
    select run.workspace_id into expected_workspace_id
    from public.discovery_runs run
    join public.discovery_segments segment on segment.id = new.discovery_segment_id
  ...
  elsif tg_table_name = 'discovery_coverage_snapshots' then
    select workspace_id into expected_workspace_id from public.discovery_segment_runs
    where id = new.discovery_segment_run_id and discovery_run_id = new.discovery_run_id;
  elsif tg_table_name = 'discovery_gaps' then
    select workspace_id into expected_workspace_id from public.discovery_runs
    where id = new.discovery_run_id;
  elsif tg_table_name = 'discovery_gap_actions' then
    select workspace_id into expected_workspace_id from public.discovery_gaps
    where id = new.discovery_gap_id;
  elsif tg_table_name = 'discovery_usage_events' then
    select workspace_id into expected_workspace_id from public.discovery_runs
    where id = new.discovery_run_id;
  else return new;
  end if;
...
begin
  ...
  if not exists (
    select 1
    from public.discovery_segment_runs segment_run
    join public.discovery_segments segment on segment.id = segment_run.discovery_segment_id
    join public.discovery_runs run on run.id = segment_run.discovery_run_id
...
begin
  foreach table_name in array array[
    'discovery_plans', 'discovery_segments', 'discovery_source_plans',
    'discovery_runs', 'discovery_segment_runs', 'discovery_queries',
    'discovery_coverage_snapshots', 'discovery_gaps', 'discovery_gap_actions',
...
begin
  ...
  insert into public.discovery_plans (
  ...
  do update set content_hash = public.discovery_plans.content_hash
  ...
  for segment in select * from jsonb_array_elements(target_plan->'segments') loop
    insert into public.discovery_segments (
    ...
    if saved_segment.id is null then
      select * into saved_segment from public.discovery_segments
    ...
    for provider in select * from jsonb_array_elements(route->'providers') loop
      ...
      insert into public.discovery_source_plans (
...
returns public.discovery_runs
language plpgsql security definer set search_path = public as $$
declare plan public.discovery_plans;
declare saved_run public.discovery_runs;
begin
  if not public.is_workspace_admin(target_workspace_id) then raise exception 'Forbidden'; end if;
  select * into plan from public.discovery_plans
  ...
  update public.discovery_plans set status = 'running' where id = plan.id;
  return saved_run;
end;
$$;
...
returns public.discovery_coverage_snapshots
language plpgsql security definer set search_path = public as $$
declare saved_snapshot public.discovery_coverage_snapshots;
declare gap jsonb;
declare action jsonb;
declare saved_gap public.discovery_gaps;
declare run_status text;
begin
  if not public.is_workspace_admin(target_workspace_id) then raise exception 'Forbidden'; end if;
  insert into public.discovery_coverage_snapshots (
  ...
  for gap in select * from jsonb_array_elements(target_gaps) loop
    ...
    for action in select * from jsonb_array_elements(gap->'recommendedActions') loop
      insert into public.discovery_gap_actions (
  ...
  update public.discovery_segment_runs set
  ...
  update public.discovery_segments set status =
  ...
  where id = (
    select discovery_segment_id from public.discovery_segment_runs
...
returns public.discovery_segment_runs
language plpgsql security definer set search_path = public as $$
declare saved public.discovery_segment_runs;
begin
  if not public.is_workspace_admin(target_workspace_id) then raise exception 'Forbidden'; end if;
  insert into public.discovery_segment_runs (
  ...
  do update set pass_number = public.discovery_segment_runs.pass_number
  returning * into saved;
  update public.discovery_segments set status = 'running' where id = target_segment_id;
  return saved;
end;
$$;
...
begin
  ...
  for query_record in select * from jsonb_array_elements(target_queries) loop
    insert into public.discovery_queries (
