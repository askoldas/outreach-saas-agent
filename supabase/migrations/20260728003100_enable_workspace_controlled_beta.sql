-- WP-21 controlled activation. Requires an explicit benchmark waiver and preserves
-- the rollback/readability guarantees from migration 30.
...
begin
  ...
  if reviewable_run_count = 0 then
    raise exception 'Controlled beta requires at least one reviewable V2 Campaign Run';
  end if;
  ...
  if unresolved_entity_case_count > 0 then
    raise exception
      'Controlled beta is blocked by % unresolved entity-resolution case(s)',
      unresolved_entity_case_count;
  end if;
  ...
  insert into public.intelligence_rollout_audit_events (
    ...
    jsonb_build_object(
      ...
      'reviewableV2RunCount', reviewable_run_count,
      'unresolvedEntityCaseCount', unresolved_entity_case_count,
