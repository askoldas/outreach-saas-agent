update public.profile_clarification_questions
set skip_allowed = true,
    updated_at = now()
where skip_allowed = false;

alter table public.profile_clarification_questions
  drop constraint if exists profile_clarification_questions_optional_check;

alter table public.profile_clarification_questions
  add constraint profile_clarification_questions_optional_check
  check (skip_allowed);
...
declare
  ...
  consistency_recommendation text;
begin
  ...
  if draft_record.state not in ('ready_for_review', 'needs_input') then
    raise exception 'Company Intelligence draft is not ready for publication.';
  end if;

  select task_run.output_json->>'publishRecommendation'
  into consistency_recommendation
  from public.profile_task_runs task_run
  where task_run.workspace_id = target_workspace_id
    and task_run.profile_draft_id = target_profile_draft_id
    and task_run.task_id = 'profile.consistency_audit'
    and task_run.status = 'completed'
  order by task_run.completed_at desc nulls last, task_run.created_at desc
  limit 1;

  if coalesce(consistency_recommendation, 'invalid') = 'invalid' then
    raise exception 'Company Intelligence consistency audit is invalid.';
  end if;
  if not exists (
