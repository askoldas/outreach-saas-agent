alter table public.candidate_qualification_batch_members_v2
  add column company_intelligence_version_id uuid
...
update public.candidate_qualification_batch_members_v2 member
set company_intelligence_version_id = assessment.company_intelligence_version_id
from public.commercial_relationship_assessment_versions_v2 assessment
where assessment.id = member.commercial_relationship_assessment_version_id
  and member.company_intelligence_version_id is null;

update public.candidate_evaluation_versions evaluation
set company_intelligence_version_id = assessment.company_intelligence_version_id
from public.commercial_relationship_assessment_versions_v2 assessment
where assessment.id = evaluation.commercial_relationship_assessment_version_id
  and evaluation.company_intelligence_version_id is null;

create index candidate_qualification_members_company_intelligence_idx
  on public.candidate_qualification_batch_members_v2(
    workspace_id, company_intelligence_version_id
  ) where company_intelligence_version_id is not null;

create index candidate_evaluations_company_intelligence_idx
  on public.candidate_evaluation_versions(
    workspace_id, company_intelligence_version_id
  ) where company_intelligence_version_id is not null;
...
begin
  ...
  loop
    ...
    if (member.commercial_relationship_assessment_version_id is not null
      ...
      or (member.company_intelligence_version_id is not null
        and member.company_intelligence_version_id <>
          assessment.company_intelligence_version_id)
    then
      raise exception 'Qualification Core Intelligence binding changed after freeze.';
    end if;
    update public.candidate_qualification_batch_members_v2
    set
      commercial_relationship_assessment_version_id = assessment.id,
      company_intelligence_version_id = assessment.company_intelligence_version_id
    where id = member.id;
    ...
    set
      ...
      compiled_snapshot_json = jsonb_set(
        ...
        to_jsonb(assessment.company_intelligence_version_id),
        true
      )
