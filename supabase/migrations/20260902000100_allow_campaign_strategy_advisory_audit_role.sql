-- The advisory-delta task replaced the historical strategy compiler model call.
-- Keep its truthful task role in the immutable AI audit instead of relabelling it.
...
begin
  ...
  loop
    if coalesce(request_item->>'role', '') not in (
      ...
      'campaign.strategy_advisory_delta'
    ) then
